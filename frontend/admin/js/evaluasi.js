...
async function openEvaluationModelDetail(model) {
    ...
    if (resultsBody) resultsBody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#999;">Loading...</td></tr>';
    ...
    let results = [];
    try {
        const res = await fetch(API_BASE + '/evaluasi/model-epochs/' + model.id);
    ...
    // Fallback to local history if API returned nothing (for backward compatibility if needed, though mostly superseded by DB)
    if (!results.length) {
        var hist = findHistoryByModel(model);
    ...
    renderEvaluationEpochMetrics(results, resultsBody, avgFoot, confusionSection, confusionMeta, confusionTable, confusionEmpty);
}
