/** Nilai `jenis` yang diizinkan pada CSV / raw_data (3 kelas). */
const ALLOWED_JENIS = ["kata kerja", "kata benda", "kata sifat"];
...
async function uploadDataset() {
    ...
    try {
        ...
        const invalidJenis = rows.filter((r) => {
            const j = (r.jenis || "").trim().toLowerCase();
            return j && !ALLOWED_JENIS.includes(j);
        });
        if (invalidJenis.length > 0) {
            const samples = [
                ...new Set(invalidJenis.slice(0, 5).map((r) => r.jenis)),
            ];
            alert(
                `CSV contains ${invalidJenis.length} row(s) with invalid jenis.\n` +
                    `Allowed: ${ALLOWED_JENIS.join(", ")}.\n` +
