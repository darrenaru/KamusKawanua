"""
Processing (training / prediksi) khusus IndoBERT.

Path: ``/processing/indobert/...`` — selain itu, endpoint lama
``/processing/train/indobert`` tetap didukung.
"""
...
from backend.processing.service import predict_indobert_softmax, train_indobert_softmax

router = APIRouter(prefix="/processing/indobert", tags=["processing-indobert"])


@router.post("/train", response_model=IndoBertTrainResponse)
def train_indobert_dedicated(req: IndoBertTrainRequest):
    try:
...
def train_indobert_dedicated_async(req: IndoBertTrainRequest):
...
def train_indobert_dedicated_status(job_id: str):
    """Alias status job (storage sama dengan ``/processing/train/status/{job_id}``)."""
    job = get_job(job_id)
    if not job:
...
def predict_indobert_dedicated(req: IndoBertPredictRequest):
    try:
